...
pub fn ChatPanel(conversation_id: Signal<ObjectId>, user_token: Signal<String>) -> Element {
    ...
    let mut handle_send_query = {
        move || {
            if !input_query().is_empty() && selected_trip().is_some() {
                thinking.set(true);
            ...
            div {
                ...
                select {
                    ...
                    for trip in trips().iter() {
                        option {
                            value: "{trip.id}",
                            selected: trip.id == selected_trip().unwrap_or_default().id,
            ...
            div {
                ...
                input {
                    class: format!(
                        ...
                        if dark_mode { "bg-gray-700 text-white border-gray-600" } else { "border-gray-300" }
                    ),
                    r#type: "text",
